...
python src/train.py --exp-name $exp_name --task-type ${task_type} --loss-type ${loss_type} \
    ...
    --model-save-dir output/training_simclue-triplet_default \
...
python train.py --exp-name $exp_name --task-type ${task_type} --loss-type ${loss_type} \
    ...
    --model-save-dir output/training_simclue-triplet_triplet \
    > logs/train.log-${exp_name}-${task_type}-${loss_type} 2>&1
...
python train.py --exp-name $exp_name --task-type ${task_type} --loss-type ${loss_type} \
    ...
    --model-save-dir output/training_simclue-qmc_default \
    > logs/train.log-${exp_name}-${task_type}-${loss_type} 2>&1
...
python train.py --exp-name $exp_name --task-type ${task_type} --loss-type ${loss_type} \
    ...
    --model bert-base-chinese --max-seq-len 256 --device cuda:0 \
    --num-epochs 4 --train-batch-size 32 --eval-batch-size 32 --learning-rate 2e-05 \
    --model-save-dir output/training_simclue-qmc_ranking \
    > logs/train.log-${exp_name}-${task_type}-${loss_type} 2>&1
...
python train.py --exp-name $exp_name --task-type ${task_type} --loss-type ${loss_type} \
    ...
    --model bert-base-chinese --max-seq-len 256 --device cuda:0 \
    --num-epochs 4 --train-batch-size 32 --eval-batch-size 32 --learning-rate 2e-05 \
    --model-save-dir output/training_simclue-qmc_logistic \
    > logs/train.log-${exp_name}-${task_type}-${loss_type} 2>&1
...
python train.py --exp-name $exp_name --task-type ${task_type} --loss-type ${loss_type} \
    ...
    --model bert-base-chinese --max-seq-len 256 --device cuda:0 \
    --num-epochs 4 --train-batch-size 32 --eval-batch-size 32 --learning-rate 2e-05 \
    --model-save-dir output/training_simclue-qmc_cosine \
    > logs/train.log-${exp_name}-${task_type}-${loss_type} 2>&1
EOF

# SimCLUE
log_file=logs/eval.log-zh_sup_general_v3.log
model_list=csts_base#output/training_csts_benchmark-sts,csts_best#output/training_csts_allnlizh_pkuparaph_pawsx-sts_default,simclue_v1#output/training_simclue-qmc_default,simclue_v2#output/training_simclue-qmc_ranking,simclue_v3#output/training_simclue-triplet_default,simclue_v4#output/training_simclue-triplet_triplet
...
nohup python src/eval.py --model-list ${model_list} --eval-list ${eval_list_simclue} --device cuda:0 --batch-size 64 > ${log_file} 2>&1 &
